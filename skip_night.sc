...
if( bed_mode,
	...
	__on_player_breaks_block(player, block) -> (
		if(global_bed_block && 
			...
			pos(block)==pos_offset(pos(global_bed_block), property(global_bed_block, 'facing'), 1)
			),
			_reset_bed_block()
		);
	);
...
_set_bed_block(block, player) -> (
	...
	global_bed_block_particles_pos = pos_offset(pos(block), property(block, 'facing'), 1) + [0.5, 0, 0.5];
);
...
__on_tick() -> (
	if(global_skip_state && !(world_time()%global_refresh_rate),
		_skip()
	);
	if( global_bed_block,
		particle('portal', global_bed_block_particles_pos, 3, 0.5, 0);
		
		if( !(world_time()%global_refresh_rate) && global_bed_block != block(pos(global_bed_block)),
			_reset_bed_block();
		);
	);
