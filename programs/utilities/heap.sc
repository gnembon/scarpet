/script run 
	...
	'get methods
	 there is no need to add return which can add extra lag';
	...
	compare_node(node1, node2) -> node1 - node2;

	'heap operations';
	heap_siftup(heap_name, index) -> (
		temp = heap_get(heap_name, index);
		parent_index = heap_get_parent_index(index);
		'Loop of size 63 should be large enough for most heap sizes. The actual loops size should be floor(log(length(var(heap_name))) / log(2) + 1)';
		while (index > 0 && compare_node(temp, heap_get(heap_name, parent_index)) < 0, 
			63,
	...
	heap_siftdown(heap_name, index) -> (
		...
		while (left_index < length(var(heap_name)) && compare_node(temp, heap_get(heap_name, left_index)) > 0 ||
			right_index < length(var(heap_name)) && compare_node(temp, heap_get(heap_name, right_index)) > 0, 
			63,
			
			if (compare_node(heap_get(heap_name, left_index), heap_get(heap_name, right_index)) > 0 && right_index < length(var(heap_name)), 
	...
	heap_pop_min(heap_name) -> (
		if (length(var(heap_name)) == 0,
			...
			min_node = element(var(heap_name), 0);
			if (length(var(heap_name)) == 1, 
				var(heap_name) = l(),
				
				put(var(heap_name), 0, element(var(heap_name), -1));
	...
	heap_change_node(heap_name, index, new_node) -> (
		...
		if (compare_node(old_node, new_node) < 0,
			heap_siftdown(heap_name, index),
	...
	heap_search(heap_name, node) -> (
		result = -1;
		for (var(heap_name), , if (compare_node(_, node) == 0, result = _i; true));
		result
	);
	...
	heap_remove(heap_name, index) -> (
		if (length(var(heap_name)) == 1,
			...
			old_node = heap_get(heap_name, index);
			put(var(heap_name), index, element(var(heap_name), -1));
			var(heap_name) = slice(var(heap_name), 0, length(var(heap_name)) - 1);
			if (compare_node(old_node, heap_get(heap_name, 0)) < 0,
				heap_siftdown(heap_name, index),
