/script run 
	...
	hashset_hasitem(map_name, item) -> (
		hashcode = hash(item);
		hashcode < length(var(map_name)) && first(element(var(map_name), hashcode), equals(_, item)) != null
	);
	...
	hashset_put(map_name, item) -> (
		...
		if (hashcode >= length(var(map_name)),
			...
			if ( first(list, if (equals(_, item), put(list, _i, item; true))) == null,
				list += item;
