...
// Currently zombies are not necessarily in cave to be killed 
// And I have no plan to implement "if in cave" checks

// Be ware zombies that users want to keep might be removed if they are holding items listed in the if condition
...
__command() -> (
    ...
    count = 0;
    l (x, y, z) = pos(player());
    for (monsters = entity_area('zombie', x,y,z, 50,50,50), 
    ...
    print ('Successfully removed ' + count + ' zombie(s).');
    null
)